...
//        log.info("Данные считаны url database: " + url);
    ...
    public List selectTable(String query) throws SQLException {
        ...
        while (rSet.next()) {        // пока есть нехе
            ...
            for (int k = 1; k < (columnNumber + 1); k++) {
                ...
                if (rSet.getObject(k) != null) {
                    ...
                    if (meta.getColumnType(k) == 93) {              // зачем?
                        value = value.substring(0, value.length() - 2);
                    }
                }

                value = value.trim();       // обрезать пробелы
                resultSet.add(value);       // добавить в переменную где все значения
            }
